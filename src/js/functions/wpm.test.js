import { textDuration, wpmFromLetters } from './wpm';
...
describe('textDuration()', () => {
    ...
    it('should calculate the chunk reading time', () => {
        ...
        expect(textDuration(textContaining500Characters, 100)).toBe(1 * 60 * 1000); // 100 WPM = 500 characters in one minnute
    });

    it('should take into account the physical eye limitations', () => {
        const veryShortChunk = "is";
        expect(textDuration(veryShortChunk, 500)).toBe(275); // See implementation
    });
    ...
    it('should ignore HTML tags in the chunk text', () => {
        ...
        const durationWithTags = textDuration(textWithTags, 100)
        const durationWithoutTags = textDuration(textWithoutTags, 100)
